...
매일 아침 미국·유럽·아시아 증시 뉴스를 모아 브리핑으로 정리하고 디스코드로 보냅니다. 생성한 문장이 검색 결과에 근거하는지 확인하는 **그라운딩 검증** 단계가 들어 있습니다. LangGraph로 만든 8노드 파이프라인입니다.
...
"검색 결과 읽고 브리핑 써줘"를 한 번에 시키면 잘 안 됩니다. 검색 원문 4~6건이 컨텍스트를 수만 토큰씩 잡아먹는 상태에서 요약·분석·작문을 동시에 시키면, 원문 뒷부분을 흘리고 숫자를 틀리고 검색에 없던 내용을 만들어냅니다. 전형적인 **할루시네이션**이고, 모델이 작을수록 심합니다.

그래서 **한 노드가 한 가지 일만** 하게 나눴습니다. 검색 계획, 사실 추출, 작문, 검증, 교정을 따로 돌리고, 각 노드는 앞 노드 출력만 봅니다. 검색 원문을 직접 보는 노드는 `synthesizer` 하나뿐이라 뒷단으로 갈수록 컨텍스트가 가벼워집니다.
...
| `search` | `ToolNode`로 검색 tool_call 을 병렬 팬아웃 | |
| `synthesizer` | 원문에서 클레임을 추출하고, 클레임 간 인과·대조를 표시한다 | ● |
| `writer` | 추출된 클레임만으로 작문한다. 검색 원문은 보지 않는다 | ● |
| `checker` | 클레임 단위로 원문 근거와 대조해 판정만 한다 (구조화 출력) | ● |
| `reviser` | 판정에 따라 최소 편집으로 고치고 인용을 삽입한다 | ● |
| `reporter` | 파일로 저장하고 디스코드로 보낸다 | |
| `cleaner` | `RemoveMessage` 로 상태를 비운다 (다음 회차용) | |

---

## 그라운딩 검증

`writer`가 쓴 글을 `checker`가 **클레임 단위로 분해**해 검색 원문 색인과 대조합니다. 판정은 NLI 방식의 3분류인 `SUPPORTED` / `CONTRADICTED` / `NOT_ENOUGH_INFO` 이고, Pydantic 스키마를 건 구조화 출력으로 받습니다. `reviser`는 그 판정에 따라 문제가 있는 구간만 손댑니다.
...
판정과 교정을 나눈 이유는 한 번에 둘 다 시키면 양쪽 다 품질이 떨어지기 때문입니다. **클레임 분해 → 근거 인용 후 판정 → 최소 편집** 순서를 따릅니다. 판정보다 근거를 먼저 쓰게 하는 건, 순서가 반대면 모델이 결론에 맞춰 근거를 지어내기 때문입니다.
...
프롬프트에 써놔도 안 지켜지는 게 있습니다. 그런 건 코드에서 강제합니다.
...
| 출처 URL을 빠뜨리거나 바꿔 쓴다 | 검증 색인을 코드에서 만들어 주입한다 |
| "없음"만 적힌 빈 섹션을 만든다 | 보내기 직전에 제목까지 지운다 |

### 데이터는 시스템 프롬프트에 넣지 않는다

시스템 메시지에는 역할과 규칙만 넣고, 실제 데이터는 `HumanMessage`로 줍니다. 데이터를 시스템 프롬프트에 넣으면 모델이 그걸 few-shot 예시 자리로 해석해 "자료를 주세요"라고 되묻습니다. 시스템 프롬프트가 회차마다 고정되므로 프롬프트 캐싱에도 유리합니다.
