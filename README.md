...
매일 아침 미국·유럽·아시아 증시 뉴스를 모아 **사실검증을 거친** 브리핑으로 정리하고 디스코드로 보냅니다. LangGraph로 만든 8노드 파이프라인입니다.
...
LLM은 OpenAI 호환 엔드포인트라면 무엇이든 붙습니다. `.env`의 주소만 바꾸면 됩니다.

---

## 왜 이렇게 나눴나

"검색 결과 읽고 브리핑 써줘"를 한 번에 시키면 세 가지가 같이 무너집니다. 원문을 끝까지 안 읽고, 수치의 소수점을 흘리고, 근거 없는 문장을 지어냅니다. 모델이 작을수록 심합니다.

그래서 **한 노드가 한 가지 일만** 하게 쪼갰습니다. 검색 계획, 사실 추출, 글쓰기, 검증, 교정이 각각 따로 돌고, 서로는 앞 노드의 결과만 봅니다.
...
`writer`가 쓴 글을 `checker`가 주장 단위로 쪼개서 검색 원문 색인과 하나씩 대조합니다. 판정은 `SUPPORTED` / `CONTRADICTED` / `NOT_ENOUGH_INFO` 셋 중 하나고, `reviser`가 그 판정대로 틀린 부분만 고칩니다.
...
판정과 수정을 굳이 나눈 이유가 있습니다. 한 번에 둘 다 시키면 양쪽 다 대충 합니다. 사실검증 연구에서 정리된 **주장 쪼개기 → 근거 먼저 인용 → 최소한만 수정** 순서를 그대로 따랐습니다.

**나가는 결과물**

> ### 오늘 한눈에
> 미국 증시는 AI 관련 기술주 강세에 힘입어 상승 마감했습니다. 7월 소비자물가지수가 예상을 밑돌며 연준의 금리 동결 기대가 다시 힘을 얻었습니다.
>
> ### 미국 증시
> S&P 500은 0.3% 올라 7,748.50에 마감했고, 나스닥 종합지수도 0.54% 상승한 26,588.49를 기록했습니다([Zacks](https://example.com), [Reuters](https://example.com)). 다만 다우존스 산업평균지수는 0.06% 내린 53,762.05로 마감해 지수별로 온도차가 있었습니다([TS2](https://example.com)).
...
프롬프트에 써놔도 안 지켜지는 것들이 있습니다. 그런 건 코드가 처리합니다.
...
| 출처 URL을 빠뜨리거나 바꿔 쓴다 | 검증용 색인을 코드가 만들어 넘긴다 |
| "없음"만 적힌 빈 섹션을 만든다 | 보내기 직전에 제목까지 지운다 |

### 데이터는 시스템 프롬프트에 넣지 않는다

시스템 메시지에는 역할과 규칙만 넣고, 실제 데이터는 사용자 메시지로 줍니다. 데이터를 시스템 프롬프트 안에 끼워 넣으면 모델이 그걸 예시 자리로 읽고 "자료를 주세요"라고 되묻습니다.

### 어제 브리핑을 참고 자료로 붙인다

`reports/`에 쌓인 직전 브리핑을 `planner`에 **`HumanMessage`로** 붙입니다. `AIMessage`로 붙이면 모델이 자기가 이미 답한 걸로 알고 검색을 건너뜁니다.

이렇게 해두면 어제 다룬 이슈는 그 뒤 어떻게 됐는지 찾고, 어제 못 구한 항목은 다시 찾습니다.
...
`.github/workflows/daily-briefing.yml`이 매일 10시(KST)에 돕니다. GitHub 러너가 그때만 켜졌다 꺼지므로 상시 서버가 필요 없습니다.

만들어진 브리핑은 `reports/`에 커밋됩니다. 러너는 매번 새 머신이라, 다음 회차의 `planner`가 어제 것을 읽으려면 저장소에 남아 있어야 하기 때문입니다.
...
`dev`는 `DISCORD_WEBHOOK_URL`만 따로 가지고, 나머지는 저장소 값을 그대로 씁니다. 시험 삼아 돌린 결과가 운영 채널로 나가거나 저장소에 커밋되는 일이 없습니다.
...
프롬프트를 조립하는 빌더는 따로 만들지 않았습니다. LangChain의 `ChatPromptTemplate`이 그 역할을 합니다. 노드 폴더는 문구와 템플릿 정의만 갖고, 변수 채우기와 모델 연결은 `template | model`이 처리합니다.
